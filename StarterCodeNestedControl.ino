...
//   float Di = 0.25;
   ...
   vdL = Kp*(-angleRad + Di*(-totalDistanceLeft)) + Ki*(-angleRadAccum + Di*(-dAccumL));
   vdR = Kp*(-angleRad + Di*(-totalDistanceRight)) + Ki*(-angleRadAccum + Di*(-dAccumR));

   vdaccumL = vdL*delta_t + vdaccumL;
   vdaccumR = vdR*delta_t + vdaccumR;
   dAccumL = totalDistanceLeft*delta_t + dAccumL;
   dAccumR = totalDistanceRight*delta_t + dAccumL;
   Serial.print(dAccumL);
   Serial.print(", ");
   Serial.println(dAccumR);
   
   pwmL = Jp*(vdL-vL) + Ji*(vdaccumL-totalDistanceLeft);
   pwmR = Jp*(vdR-vR) + Ji*(vdaccumR-totalDistanceRight);
   leftMotorPWM = pwmL;
   rightMotorPWM = pwmL;
}
