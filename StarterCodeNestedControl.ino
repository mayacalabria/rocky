...
//   float vd_l;
   ...
   int pwmL;
   int pwmR;
//   vd_r = Kp*(-angleRad - 0.1*totalDistanceRight) + Ki*(-angleRadAccum - 0.1*totalDistanceRight);
//   vd_l = Kp*(-angleRad - 0.1*totalDistanceLeft) + Ki*(-angleRadAccum - 0.1*totalDistanceLeft);
   vd = Kp*(-angleRad) + Ki*(-angleRadAccum);
//   This code currently doesn't work. A professor should be able to help if you tell them that we are working on
//   adding a distance control loop. This is a third feedback loop.

   vdaccum = vd*delta_t + vdaccum;
//   vdaccum_r =+ vd_r*delta_t;
//   vdaccum_l =+ vd_l*delta_t;
//   pwmL = Jp*(vd_l-vL) + Ji*(vdaccum_l-totalDistanceLeft);
//   pwmR = Jp*(vd_r-vR) + Ji*(vdaccum_r-totalDistanceRight);
   pwmL = Jp*(vd-vL) + Ji*(vdaccum-totalDistanceLeft);
   pwmR = Jp*(vd-vR) + Ji*(vdaccum-totalDistanceRight);
   leftMotorPWM = pwmL;
   rightMotorPWM = pwmR;
}
...
//  vd = -Kp*angle_rad - Ki*angle_rad_accum;
