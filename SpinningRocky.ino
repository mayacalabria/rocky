...
void updatePWMs(float totalDistanceLeft, float totalDistanceRight, float vL, float vR, float angleRad, float angleRadAccum, float delta_t) {
   ...
   */
   ...
   distance = (totalDistanceLeft+totalDistanceRight)/2;  
   vd = Kp*(-angleRad + Di*(-distance)) + Ki*(-angleRadAccum + Di*(-dAccum));
   vdaccum = vd*delta_t + vdaccum;
   dAccum = distance*delta_t + dAccum;
   
   pwm = Jp*(vd-((vL+vR)/2)) + Ji*(vdaccum-distance); 
   leftMotorPWM = pwm*0.5;
   rightMotorPWM = 1.5*pwm;
}
